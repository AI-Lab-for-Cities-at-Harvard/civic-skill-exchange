...
The contract between `generalize-skill` and `localize-skill`. Both carry a copy;
nothing else depends on it, so it can change — but it has to change in both
places at once, or one side will write a file the other cannot read.

`generalize` writes this file. `localize` fills it, and adds one of its own: a
`<org>.profile.yml` holding the answers that are true of an organization rather
than of a skill. See the last section.
...
## `<org>.profile.yml` — the same shape, one level up

Written by `localize`, not by `generalize`, and the reason a second adoption is
cheap.

Some answers belong to an organization rather than to a skill: the jurisdiction,
the open-data portal, the fiscal year, who signs off, what the public works
department is called. Every skill this organization adopts asks for them again,
and asking again is how an exchange loses people after the first skill.
...
profile:
  jurisdiction:
    what: "Legal name of the organization."
    exact: false
    value: "Town of Brookline"
  open_data_portal:
    what: "Where published datasets live."
    exact: true
    value: "https://data.brooklinema.gov"
  approving_official:
    what: "Who signs off on analysis leaving the team."
    exact: false
    value: "Town Administrator"
```

Same entry shape as `slots`, so filling logic is shared. When localizing, read
the profile first and treat matching keys as already answered — then write back
anything newly learned that is org-wide rather than skill-specific.

The test for whether a value belongs here: **would the answer change if this
organization adopted a completely different skill?** If no, it is profile. A
dataset identifier is not; a fiscal year is.

Provenance travels with each value — `asked`, `derived`, `defaulted` — because a
value inherited into a fifth skill should not silently gain confidence it never
had.
