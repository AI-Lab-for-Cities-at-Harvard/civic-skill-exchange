...
| `.claude-plugin/`, except `plugin.json` | Generated by the registry from the listing itself. An author's copy would shadow it and describe the plugin as something other than what the catalogue shows. `.claude-plugin/plugin.json` is the one allowed path, because the registry writes that one: `scripts/build_marketplace.py` derives it from the author's own frontmatter, and `build_marketplace.py --check` fails a pull request whose committed copy differs — a blocking step in `validate.yml`, with nothing repairing a stale copy after a merge. So an author cannot reach a client through it with inline `hooks` or `mcpServers`. |
